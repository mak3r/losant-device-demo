...
# etcd peer communication between HA server nodes
resource "google_compute_firewall" "allow_etcd" {
  name    = "${local.prefix}-allow-etcd"
  network = "default"
...
# flannel VXLAN overlay between cluster nodes
resource "google_compute_firewall" "allow_flannel" {
  name    = "${local.prefix}-allow-flannel"
  network = "default"
...
# First server — bootstraps the embedded etcd HA cluster
resource "google_compute_instance" "server_init" {
  ...
  metadata = {
    ...
    user-data = templatefile("${path.module}/cloud-init-server.yaml.tpl", {
...
resource "google_compute_instance" "server_join" {
  count        = 2
  ...
  metadata = {
    ssh-keys = "ubuntu:${file(var.ssh_public_key_path)}"
    user-data = templatefile("${path.module}/cloud-init-agent.yaml.tpl", {
      k3s_channel = var.k3s_channel
      k3s_token   = local.k3s_token
      server0_ip  = google_compute_address.server.address
    })
  }
}
